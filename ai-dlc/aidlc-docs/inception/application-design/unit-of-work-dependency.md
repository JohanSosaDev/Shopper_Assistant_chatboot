...
| **Unit 3 (Handoff & Convivencia)** | ✅ (depende de M1, M3, M4, M7, models/, lib/) | ✅ parcial (orderHistory para handoff package; brand context para rollout gating) | — |
...
### IP-5: Unit 3 publica eventos al widget SFCC (handoff dispatched + rollout decision)
- **Where**: El widget SFCC del cliente final consume el endpoint público `/widget/config` (RolloutGate decide si servir Hermes o el fallback al cargar) y respuestas de `/chat` (incluye `handoffTriggered` flag para que el widget cambie UI).
...
**Regla**: si por cualquier razón Unit 1 se extiende y comprime los plazos, **degradar Unit 2 antes que Unit 3**. Sin Unit 3 no hay despliegue gradual ni kill switch (no se puede deploy seguro a producción); sin Unit 2 solo se pierde el sign-off auditable (parchearable con commit manual del prompt).
...
- En Unit 3: M5 handoff (TriggerDetector + PackageBuilder + DeliveryAdapter) y M8 RolloutGate pueden trabajarse en paralelo dentro de la unit; dashboards pueden seguir.
...
- **Schema migrations**: Unit 1 ejecuta `0001_unit1_base.sql`. Unit 2 ejecuta `0002_unit2_brand_config.sql` después de Unit 1 desplegada. Unit 3 ejecuta `0003_unit3_handoff_rollout.sql` (incluye `handoff_tickets`, `system_config` con `rollout_salt`/`hermes_enabled`/`hermes_traffic_percentage`, `alert_rules`, `system_config_audit`) después de Unit 2.
...
| Unit 3 | Kill switch: `PATCH /admin/rollout/kill-switch {enabled: false}` deja todo el tráfico al fallback humano (atención en horario o mensaje informativo); o `PATCH /admin/rollout/traffic-percentage {percentage: 0}` para sub-rollback granular. Handoff queda inactivo automáticamente porque ningún cliente entra a Hermes. *Nota*: auto-rollback por degradación de KPI degradado a Fase 2 per Unit 3 NFR-R — MVP requiere acción manual del operador alertado vía Slack. |
